    ...
    <div>
      <Box sx={mainDiv}>
        ...
        <div style={{padding:'10px 0'}}>
          <p style={{ color: `${lightTheme.headingTextColor}`, fontSize: '20px', fontWeight:'600'}}>Your Starts</p>
          <p style={{ color: `${lightTheme.dateColor}`, fontSize: '12px' }}>(FROM MAY 10,2022 TO MAY 11, 2022)</p>
          <Grid container my={3} rowGap={4}>
            <Grid item lg={3} md={6} sm={12}>
              <p style={{ color: `${lightTheme.headingTextColor}`, fontSize: '20px', fontWeight: '500' }}>Best Month</p>
              <p style={{ color: `${lightTheme.headingTextColor}`, fontSize: '18px', fontWeight: '500' }}>$7,00.00</p>
              <p style={{ color: `${lightTheme.textColor}`, fontSize: '12px' }}>In May 2022</p>
            </Grid>
            <Grid item lg={4} md={6} sm={12}>
            <p style={{ color: `${lightTheme.headingTextColor}`, fontSize: '20px', fontWeight: '500' }}>Lowest Month</p>
              <p style={{ color: `${lightTheme.headingTextColor}`, fontSize: '18px', fontWeight: '500' }}>$7,00.00</p>
              <p style={{ color: `${lightTheme.textColor}`, fontSize: '12px' }}>In May 2022</p>
            </Grid>
            <Grid item lg={4} md={6} sm={12}>
            <p style={{ color: `${lightTheme.headingTextColor}`, fontSize: '20px', fontWeight: '500' }}>Average</p>
              <p style={{ color: `${lightTheme.headingTextColor}`, fontSize: '18px', fontWeight: '500' }}>$7,00.00</p>
              <p style={{ color: `${lightTheme.textColor}`, fontSize: '12px' }}>In May 2022</p>
            </Grid>
          </Grid>
        ...
        <Grid container columnGap={4}  rowGap={4}>
          <Grid item lg={5.5} md={12} sm={12}>
            <div style={{ border: `1px solid ${lightTheme.borderColor}`, borderRadius: '8px' }}>
              {
                TotalProfitLossData.map((data, index) => {
                  return (
                    <>
                    <div key={index} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'15px 15px'}}>
                      <p style={{ fontWeight: '500', fontSize: '16px', color: `${lightTheme.headingTextColor}` }}>{data.name}</p>
                      <p style={{ fontWeight: '400', fontSize: '16px', color: `${lightTheme.textColor}` }}>{data.value}</p>
            </div>
                  <Divider /> 
                    </>
                  )
